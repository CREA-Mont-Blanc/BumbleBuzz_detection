...
def tagging_validate(Df, fewlabels=fewlabels, dict_allcats=dict_allcats):
    ...
    # macro_cat = {
    # 'geophony':['Wind', 'Rain', 'River', 'Wave', 'Thunder'],'biophony': ['Bird', 'Amphibian', 'Insect', 'Mammal', 'Reptile'], 'anthropophony': ['Walking', 'Cycling', 'Beep', 'Car', 'Car honk', 'Motorbike', 'Plane', 'Helicoptere', 'Boat', 'Others_motors', 'Shoot', 'Bell', 'Talking', 'Music', 'Kitchen sounds', 'Rolling shutter'],
    # 'buzz':['Buzz'],
    # 'domesticanimals':['Dog bark','Rooster']
    # }
    # Simple macro_cat using only categories that exist as columns in the CSV output
    # Based on what we actually have in TaggingCategory.csv columns
    macro_cat = {
        "Group_buzz": ["fly_housefly", "bee_wasp"],  # Combination of both
        "Group_geophony": ["water", "wind"],
        "Group_anthropophony": ["motor_vehicle", "aircraft", "human_voice"],
    }

    ### and now we will calculate the max probability in each of the macro categories
    ...
    for cursubcat in macro_cat.keys():
        ...
        missing_labels = [
            label for label in curlabels if label not in available_columns
        ]
        available_labels = [label for label in curlabels if label in available_columns]

        if missing_labels:
            print(f"Missing labels for {cursubcat}: {missing_labels}")

        if not available_labels:
            print(f"No available labels for category {cursubcat}, skipping...")
            continue

        # print(f"Using available labels: {available_labels}")

        curDf = Df_new[available_labels]
        Df_new[cursubcat] = curDf.max(axis=1)
        # keep only the max probability
        # Don't drop the original columns as they might be needed for other categories
        # Df_new.drop(columns=available_labels, inplace=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script to regroup tagging output into a csv file"
    )
    parser.add_argument("--input", default=None, type=str, help="Path to pkl file")
    parser.add_argument(
        "--save_path",
        default="/bigdisk2/meta_silentcities/tests_eco",
        type=str,
        help="Path to save output csv and pkl",
    )
